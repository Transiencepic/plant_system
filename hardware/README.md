...
\- \*\*树莓派\*\*：运行主控程序 `raspberry\_pi/main.py`，负责与 Arduino 通信、上传数据到后端、提供拍照和浇水接口

\- \*\*Arduino\*\*：运行 `arduino/plant\_monitor.ino`，连接传感器并控制水泵
...
1\. 用 Arduino IDE 打开 `arduino/plant\_monitor.ino`
...
在 raspberry\_pi/main.py 中设置正确的串口号（如 /dev/ttyUSB0）
